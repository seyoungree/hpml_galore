...
class GaLoreProjector:
        ...
        if self.proj_type == 'std': 
            if full_rank_grad.shape[0] >= full_rank_grad.shape[1]:
                if self.ortho_matrix is None or iter % self.update_proj_gap == 0:
                    ...
                    if QUANTIZE:
                        self.ortho_matrix, self.quantization_state = self._quantize(self.ortho_matrix, QUANTIZE, BLOCKSZ)  
                if QUANTIZE:
                    self.ortho_matrix = self._dequantize(self.ortho_matrix, QUANTIZE, self.quantization_state, full_rank_grad.dtype) # dequantize
                low_rank_grad = torch.matmul(full_rank_grad, self.ortho_matrix.t())
                if QUANTIZE:
                    self.ortho_matrix, self.quantization_state = self._quantize(self.ortho_matrix, QUANTIZE, BLOCKSZ) # quantize back for storage
            else:
                if self.ortho_matrix is None or iter % self.update_proj_gap == 0:
                ...
                if QUANTIZE:
                    self.ortho_matrix, self.quantization_state = self._quantize(self.ortho_matrix, QUANTIZE) # quantize back for storage
            ...
            if self.ortho_matrix is None or iter % self.update_proj_gap == 0:
                ...
                # quantize orthomatrices
                # bitsandbytes.functional.quantize(self.ortho_matrix)
            else:
                # dequantize orthomatrices
                # bitsandbytes.functional.dequantize(self.ortho_matrix)
                low_rank_grad = torch.matmul(self.ortho_matrix[0].t(), full_rank_grad) @ self.ortho_matrix[1].t()
                # quanitze orthomatrices
                # bitsandbytes.functional.quantize(self.ortho_matrix)
    ...
    def project_back(self, low_rank_grad):
        ...
        if self.proj_type == 'std':
            if QUANTIZE:
                self.ortho_matrix = self._dequantize(self.ortho_matrix, QUANTIZE, self.quantization_state, low_rank_grad.dtype) # dequantize
